...
function handleUserActivity() {
    ...
    // Set a timeout to reset user activity status after 10 seconds
    setTimeout(() => {
        isUserActive = false;
    }, 10000);
}
...
let refreshTimer = setTimeout(refreshPage, 10000);

// Periodic refresh every 10 seconds if not scrolling, moving, or typing
setInterval(() => {
    if (!isUserActive) {
        refreshPage();
    }
